...
<#--  /** default constructor */ -->
    public ${pojo.getDeclarationName()?replace(tablePrefix,"")}() {
    }

<#if pojo.needsMinimalConstructor()>	<#-- /** minimal constructor */ -->
    public ${pojo.getDeclarationName()?replace(tablePrefix,"")}(${c2j.asParameterList(pojo.getPropertyClosureForMinimalConstructor(), jdk5, pojo)}) {
...
<#-- /** full constructor */ -->
    public ${pojo.getDeclarationName()?replace(tablePrefix,"")}(${c2j.asParameterList(pojo.getPropertyClosureForFullConstructor(), jdk5, pojo)}) {
